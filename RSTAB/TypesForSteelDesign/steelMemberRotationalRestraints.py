from RSTAB.initModel import Model, clearAtributes, ConvertToDlString, GetAddonStatus, SetAddonStatus
...
class SteelMemberRotationalRestraint():
    def __init__(self,
        ...
        # Deducing RSTAB Language from steel_design_addon String:
        modelInfo = Model.clientModel.service.get_model_info()
        if modelInfo.property_addon_steel_design.split()[0] != 'Steel':
            raise Exception("WARNING: The steelMemberRotationalRestraints operates with the RSTAB Application set to English. Kindly switch RSTAB to English such that Database searches can completed successfully.")
